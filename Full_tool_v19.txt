...
class BarPropertySolverTab:
    def __init__(self, parent_frame, root):
        ...
        # Default bar thickness used only when a property has no original BDF/Excel value
    ...
    def setup_ui(self):
        ...
        f4 = ttk.LabelFrame(main, text="3. Structure Groups (from Property Excel)", padding=10)
        ...
        # ---- Section 4: Log ----
        f5 = ttk.LabelFrame(main, text="4. Log", padding=10)
        ...
        while i < len(lines):
            ...
            if line.startswith('PBARL'):
                try:
                    ...
                    should_update = pid in self.current_bar_thicknesses
                    if should_update:
    ...
    def _apply_offsets(self, bdf_path, folder, maneuver_bdf_path=None):
        ...
        try:
            ...
            for eid in self.bar_offset_elem_ids:
                ...
                bar_t = self.current_bar_thicknesses.get(pid, self.default_bar_thickness)
                bar_t = max(0.1, bar_t)
    ...
    def _write_structure_only_bdf(self, bdf_path, folder):
        """Write a copy of the given (prop-updated + offseted) BDF with INCLUDE cards
        removed, so only the structure itself remains, without needing the include files."""
        try:
            with open(bdf_path, 'r', encoding='latin-1') as f:
                lines = f.readlines()
            ...
            self.log(f"    Structure-only BDF written: {os.path.basename(output_bdf)} ({removed} include lines removed)")
            return output_bdf

        except Exception as e:
            self.log(f"    Structure-only BDF error: {e}")
            return None
    ...
    def _run_single_iteration(self, folder):
        """Write BDFs with current properties, apply offsets, run Nastran, extract stresses."""
        try:
            n_bdfs = len(self.bdf_models) if self.bdf_models else 1
            all_stresses = []

            # Step 0: Update maneuver BDF with current properties for offset geometry
            ...
            for bdf_idx, bdf_info in enumerate(self.bdf_models):
                ...
                # 1. Write BDF with current properties
                ...
                # 2b. Also write a structure-only copy (INCLUDE cards removed)
                if offset_bdf:
                    self._write_structure_only_bdf(offset_bdf, bdf_subfolder)
    ...
    def _run_solve(self):
        """Process property excel, apply offsets, and solve the base model once."""
        try:
            ...
            stresses = self._run_single_iteration(run_folder)
            self.root.after(0, lambda: self.progress_var.set(100))

            if stresses:
                abs_s = [abs(s['stress']) for s in stresses]
                self.log(f"\n  Max stress: {max(abs_s):.2f}, Avg stress: {np.mean(abs_s):.2f}")
                self.log(f"  Elements: {len(stresses)}")
